...
namespace Mapping
{
    namespace Digital
    {
        Mapper::Mapper(SwitchPro::IEventPtr from, X360::IModifierPtr to) : event_(from), modifier(to) { }

        void Mapper::Map(const SwitchPro::Controller& from, X360::Controller& to)
        {
    ...
    void Map(const Mappers& mappers, const SwitchPro::Controller& from, X360::Controller& to)
    {
...
namespace YAML
{
    ...
    bool convert<Mapping::Digital::MapperPtr>::decode(const Node& node, Mapping::Digital::MapperPtr& mapper)
    {
        ...
        auto event = fromNode.as<SwitchPro::IEventPtr>();
