...
namespace Mapping
{
    namespace Analog
    {
        ...
        template<typename FromOffsetT, typename FromStickT, typename ToOffsetT, typename ToStickT>
        void LinearMapper<FromOffsetT, FromStickT, ToOffsetT, ToStickT>::Map(const SwitchPro::Controller& from, X360::Controller& to)
        {
        ...
        template<typename FromOffsetT, typename FromStickT, typename ToOffsetT, typename ToStickT>
        void BilinearMapper<FromOffsetT, FromStickT, ToOffsetT, ToStickT>::Map(const SwitchPro::Controller& from, X360::Controller& to)
        {
            float tmpValues[2];
            for (int i = 0; i < 2; i++)
            {
...
namespace YAML
{
    ...
    bool convert<Mapping::Analog::BilinearMapperPtr<FromOffsetT, FromStickT, ToOffsetT, ToStickT>>::decode(const Node& node, Mapping::Analog::BilinearMapperPtr<FromOffsetT, FromStickT, ToOffsetT, ToStickT>& mapper)
    {
        ...
        auto fromX = fromXNode.as<SwitchPro::LinearConverter>();
        auto fromY = fromYNode.as<SwitchPro::LinearConverter>();
